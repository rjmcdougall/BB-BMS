/*
* Description :   Header file of bq_simulator BMS IC (by Texas Instruments) for Arduino platform.
...
enum bq_simulator_fet_state
{
	FET_OFF,
	FET_ON
};

enum bq_simulator_scd_thresh
{
	SCD_10,
	SCD_20,
	SCD_40,
...
} bq_simulator_temperature_t;

class bq_simulator
{
public:
	bq_simulator(uint8_t addr, HAL_ESP32 *hal, i2c_port_t port);
	void begin(void);
	void reset(void);
	...
	float getThermistorTemp(bq_simulator_thermistor);
	bq_simulator_protection_t getProtectionStatus(void);
	bq_simulator_temperature_t getTemperatureStatus(void);
	void getCellBalanceStatus(bool *cellArray);
	void getCellBalanceTimes(uint32_t *cellArray);
	void setFET(bq_simulator_fet, bq_simulator_fet_state);
	...
	void setDischargingShortcircuitProtection(bq_simulator_scd_thresh, unsigned int);
