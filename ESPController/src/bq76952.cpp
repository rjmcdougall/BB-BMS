...
bool bq76952::readDataMemory(uint16_t addr, uint8_t *data)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(BQhal->writeMultipleBytes(I2C_NUM_0, BQaddr, CMD_DIR_SUBCMD_LOW, data, 2));

    ESP_LOGD(TAG,"BQreadDataMemory Send Read Req %i", addr);
    esp_err_t ret = BQhal->readByte(I2C_NUM_0, BQaddr, CMD_DIR_RESP_START, data);
