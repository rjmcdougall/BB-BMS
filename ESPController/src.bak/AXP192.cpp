...
{
    AXPhal->writeByte(I2C_NUM_0, AXP_ADDR, Addr, Data);
}

uint8_t AXP192::Read8bit(uint8_t Addr)
{
    uint8_t value;
    AXPhal->readByte(I2C_NUM_0, AXP_ADDR, Addr, &value);
    return value;
}
...
uint16_t AXP192::Read12Bit(uint8_t Addr)
{
    ...
    ret = AXPhal->readByte(I2C_NUM_0, AXP_ADDR, Addr, &tmp_value[0]);
    if (ret != ESP_OK) {
        return 0;
    }
    ret = AXPhal->readByte(I2C_NUM_0, AXP_ADDR, Addr + 1, &tmp_value[1]);
    if (ret != ESP_OK) {
        return 0;
    }
    ret = tmp_value[0] | (tmp_value[1] << 4);
    return ret;
    //Data = ((buf[0] << 4) + buf[1]); 
}
...
uint32_t AXP192::Read24bit(uint8_t Addr)
{
    ...
    ret = AXPhal->readByte(I2C_NUM_0, AXP_ADDR, Addr + 2, &tmp_value[2]);
    if (ret != ESP_OK) {
        return 0;
    }
    ret = tmp_value[0] | (tmp_value[1] << 8) | (tmp_value[2] << 16);
    return ret;
}
...
uint32_t AXP192::Read32bit(uint8_t Addr)
{
    ...
    ret = AXPhal->readByte(I2C_NUM_0, AXP_ADDR, Addr + 3, &tmp_value[3]);
    if (ret != ESP_OK) {
        return 0;
    }
    ret = tmp_value[0] | (tmp_value[1] << 8) | (tmp_value[2] << 16) | (tmp_value[3] << 24);
    return ret;
}
...
void AXP192::SetSpkEnable(uint8_t state)
{
    ...
    ESP_LOGI(TAG, "axp: SetSpkEnable %x", data);
